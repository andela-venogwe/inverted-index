...
describe('Read book data', () => {
  ...
  it('Should return a valid JSON array', () => {
    ...
    expect(index.utility.readFile('./src/public/uploads/books.json', (data) => {
      return data;
    }))
    .toBe(undefined);
  ...
  it('Should return a JSON array which has valid string entries for keys(title, text)', () => {
    ...
    expect(index.utility.isValidJson(fileContents)).toBe(true);
    expect(index.utility.isValidJson('')).toBe(false);
    expect(index.utility.isValidJson([])).toBe(false);
    expect(index.utility.isValidJson(testJson)).toBe(false);
    expect(index.utility.isValidJson(testJson2)).toBe(false);
  });
